...
def _positive_int(value: Any) -> bool:
    # 任意非布尔正整数：不设数据库整数上界，超范围值交由存在性查询按缺失处理。
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _read_json_object(
        ...
        try:
            ...
            return None
        try:
            # 任意长度整数仅限审计序号入口；其他入口保留全局转换保护。
            parsed = json.loads(
                text,
                object_pairs_hook=_unique_object,
                parse_int=_audit_json_int if unbounded_ints else int,
            )
        except ValueError:
            return None
    ...
    def _audit_idempotency_key(self) -> str | None:
        # 审计写入口要求恰好一个合法 Idempotency-Key：缺失、重复或格式非法
        # 均为 400/invalid_request，且不查询任何资源。
        values = self.headers.get_all("Idempotency-Key")
        if values is None or len(values) != 1:
            return None
        if IDEMPOTENCY_KEY_PATTERN.fullmatch(values[0]) is None:
            return None
        return values[0]
    ...
    def _build_audit_document(
        ...
        # 反向构造预期分录：在同一冻结事务内，从入金、正金额结算、
        # 直接退款（已退款且无仲裁记录的争议）、升级与仲裁记录推导每个
        # (kind, referenceSeq) 应有的账户侧与有符号金额。
        # 零金额 pending 结算仍不产生任何分录。
        arbitrated_dispute_ids = {
            arbitration["dispute_id"] for arbitration in arbitrations.values()
        }
        expected_entries: dict[tuple[str, int], dict[str, Any]] = {}

        def settlement_attribution(settlement: Any) -> tuple[Any, Any] | None:
            # 结算关联的 SLA 须存在且评估属于同一 SLA；断裂时返回
            # (预期SLA, 实际SLA) 供 entry_attribution 差异使用。
        ...
        for deposit_seq in sorted(deposits):
            ...
            amount = deposit["amount_micros"]
            expected_entries[("deposit", deposit_seq)] = {
                "sides": [
        ...
        for settlement_seq in sorted(settlements):
            ...
            if (
                amount <= 0
                or settlement["payer_id"] is None
                or settlement["payee_id"] is None
            ):
                continue
            expected_entries[("settlement", settlement_seq)] = {
                "sides": [
                    (settlement["payer_id"], -amount),
                    (settlement["payee_id"], amount),
                ],
                "attribution": settlement_attribution(settlement),
                "attribution_seq": settlement_seq,
                "reference_broken": False,
            }
        for dispute_id in sorted(disputes_by_id):
            dispute = disputes_by_id[dispute_id]
            if (
                dispute["state"] != "refunded"
                or dispute_id in arbitrated_dispute_ids
            ):
                continue
            amount = dispute["amount_micros"]
            settlement = settlements.get(dispute["settlement_seq"])
            expected_entries[("dispute_refund", dispute["settlement_seq"])] = {
                "sides": [
                ...
                "attribution": (
                    None
                    if settlement is None
                    else settlement_attribution(settlement)
                ),
                "attribution_seq": dispute["settlement_seq"],
                # 结算记录缺失（篡改）时沿用逐笔 entry_reference 核对。
                "reference_broken": settlement is None,
            }
        for escalation_seq in sorted(escalations):
            ...
            sides = [(CLEARING_ACCOUNT_ID, escalation["amount_micros"])]
            if dispute is not None:
                sides.insert(
                    0, (dispute["payee_id"], -escalation["amount_micros"])
                )
            attribution = None
            attribution_seq = None
            if dispute is not None:
                ...
                if settlement is None:
                    ...
                    attribution_seq = escalation_seq
                else:
                    attribution = settlement_attribution(settlement)
            expected_entries[("dispute_escalation", escalation_seq)] = {
                "sides": sides,
                "attribution": attribution,
                "attribution_seq": attribution_seq,
                "reference_broken": False,
            }
        for arbitration_seq in sorted(arbitrations):
            ...
            sides = [(CLEARING_ACCOUNT_ID, -arbitration["amount_micros"])]
            if dispute is not None:
                recipient_id = (
                    dispute["payee_id"]
                ...
                sides.append((recipient_id, arbitration["amount_micros"]))
            attribution = None
            attribution_seq = None
            reference_broken = False
            if dispute is not None:
                ...
                # 争议状态与仲裁决定不一致（篡改）时沿用逐笔 entry_reference。
                reference_broken = dispute["state"] != expected_state
                settlement = settlements.get(dispute["settlement_seq"])
                attribution_seq = dispute["settlement_seq"]
                if settlement is None:
                    attribution = (None, None)
                    attribution_seq = arbitration_seq
                else:
                    attribution = settlement_attribution(settlement)
            expected_entries[("dispute_arbitration", arbitration_seq)] = {
                "sides": sides,
                "attribution": attribution,
                "attribution_seq": attribution_seq,
        ...
        for entry in entries:
            entries_by_pair.setdefault(
                (entry["kind"], entry["reference_seq"]), []
            ).append(entry)

        for pair_key in sorted(entries_by_pair.keys() | expected_entries.keys()):
            kind, reference_seq = pair_key
            pair = entries_by_pair.get(pair_key, [])
            expected = expected_entries.get(pair_key)
            if expected is None:
                # 无对应业务记录（含未知种类）的分录：业务引用无法核对，
                # 成对计数与金额守恒沿用既有差异类型。
                for entry in pair:
                    differences.append(
                        self._audit_difference(
                ...
                if total != 0:
                    differences.append(
                        self._audit_difference(
                            pair[0]["account_id"], "entry_pair_amount",
                            pair[0]["entry_seq"], reference_seq, 0, total,
                        )
                    )
                continue
            ...
            for index, entry in enumerate(pair):
                if consumed[index]:
                    continue
                if entry["account_id"] in side_accounts:
                    differences.append(
                        self._audit_difference(
                            entry["account_id"], "entry_unpaired",
                            entry["entry_seq"], reference_seq, 2, len(pair),
                ...
                for leg_index, (account, _delta) in enumerate(sides):
                    ...
                    differences.append(
                        self._audit_difference(
                            ...
                            None if entry is None else entry["entry_seq"],
                            expected["attribution_seq"],
    ...
    def _read_audit_comparison_object(self, body: bytes) -> dict[str, Any] | None:
        ...
        # 端点序号接受任意非布尔正整数，不以数据库整数上限判为非法请求；
        # 超出存储范围但格式合法者按检查点不存在处理（404/not_found）。
    ...
    def _apply_audit_comparison(
        ...
        with closing(connect(self.server.database_path)) as database:
            ...
            try:
                ...
                # 序号超出 SQLite 整数存储范围时格式仍合法，按不存在处理，不绑定参数。
                if from_seq <= INT64_MAX:
                ...
                self._append_audit_comparison_chain(
                    database,
                    comparison_seq,
                    json.dumps(
    ...
    @staticmethod
    def _append_audit_comparison_chain(
        database: Any, comparison_seq: int, response_bytes: bytes
    ) -> None:
        # 首次成功比较在原事务追加链记录：responseDigest 取首次完整响应的紧凑
        ...
        previous_chain_digest = (
            ...
            else AUDIT_CHAIN_GENESIS_DIGEST
        )
        chain_text = "\n".join(
            (
                "audit-chain-v1",
        ...
        database.execute(
            ...
            "(comparison_seq, response_digest, previous_chain_digest, chain_digest)"
            " VALUES (?, ?, ?, ?)",
            (
                comparison_seq,
                response_digest,
    ...
    def _audit_chain_page(
        self, database: Any, cut: int, last_seq: int, limit: int
    ) -> tuple[list[dict[str, Any]], str | None]:
        rows = database.execute(
            "SELECT comparison_seq, response_digest, previous_chain_digest,"
            " chain_digest FROM audit_comparison_chain"
            ...
            try:
                ...
                if cursor is None:
                    ...
                    if anchor is None:
                        ...
                        return
                entries, next_cursor = self._audit_chain_page(
                    database, cut, last_seq, limit
                )
    ...
    def _create_audit_anchor(self) -> None:
        ...
        if parse_qsl(urlsplit(self.path).query, keep_blank_values=True):
            self._json(HTTPStatus.BAD_REQUEST, {"error": "invalid_request"})
            return
        # 正文恰含 comparisonSeq（非布尔正整数，任意长度）与 chainDigest
        # （64 位小写十六进制），键不得重复。
        raw_body = self._read_raw_body()
        fields = (
            None
    ...
    def _read_audit_anchor_object(self, body: bytes) -> dict[str, Any] | None:
        ...
        # 链位置接受任意非布尔正整数，不以数据库整数上界判为非法请求；
        # 超出存储范围但格式合法者按链节点不存在处理（404/not_found）。
        if not _positive_int(comparison_seq):
            return None
        if (
        ...
        with closing(connect(self.server.database_path)) as database:
            ...
            try:
                ...
                if chain_row is None:
                    database.execute("ROLLBACK")
                    return HTTPStatus.NOT_FOUND, {"error": "not_found"}
                # 摘要须与保存的链摘要逐字一致；错配为冲突，不锚定也不消费随机数。
                if chain_row["chain_digest"] != claimed_chain_digest:
                    database.execute("ROLLBACK")
                    return HTTPStatus.CONFLICT, {"error": "conflict"}
                # 同一审计机器对同一链节点限锚定一次；不同审计机器仍可分别签名。
                existing = database.execute(
                    "SELECT 1 FROM audit_anchors"
                    " WHERE comparison_seq = ? AND auditor_machine_id = ?",
    ...
    def _get_audit_anchors(self, query: str) -> None:
        # 锚点集合分页沿用比较列表的 limit 与 cursor=cut:lastSeq。
    ...
    def _create_audit_verification(self) -> None:
        ...
        if parse_qsl(urlsplit(self.path).query, keep_blank_values=True):
            self._json(HTTPStatus.BAD_REQUEST, {"error": "invalid_request"})
            return
        # 正文恰含 comparisonSeq（非布尔正整数，任意长度），键不得重复。
        raw_body = self._read_raw_body()
        fields = (
            None
            if raw_body is None
            else self._read_audit_verification_object(raw_body)
        )
        if fields is None:
            self._json(HTTPStatus.BAD_REQUEST, {"error": "invalid_request"})
            return
        ...
        if auth is None:
            ...
            return
        body_digest = hashlib.sha256(raw_body).hexdigest()
        status, payload = self._apply_audit_verification(
            fields, auth, body_digest, idempotency_key
        )
        self._json(status, payload)
    ...
    def _read_audit_verification_object(self, body: bytes) -> dict[str, Any] | None:
        ...
        # 超出存储范围但格式合法者按比较不存在处理（404/not_found）。
        if not _positive_int(comparison_seq):
            return None
        return parsed

    def _compute_audit_verification(
        self, database: Any, comparison_bound: int, anchor_bound: int
    ) -> tuple[bool, bool, list[int], list[int], list[int]]:
        # 冻结上界后只读重放：比较响应、比较链记录与锚点均不被修正，
        # 链缺失、替换、重排或锚点签名无效只记录为异常序号。
        comparison_rows = database.execute(
            "SELECT comparison_seq, response_json FROM audit_comparisons"
            " WHERE comparison_seq <= ? ORDER BY comparison_seq ASC",
            (comparison_bound,),
        ).fetchall()
        ...
        anchor_rows = database.execute(
            "SELECT anchor_seq, comparison_seq, chain_digest, auditor_machine_id,"
            " public_key, key_version, request_time_ms, nonce, body_digest,"
            " auth_signature FROM audit_anchors WHERE anchor_seq <= ?"
            " ORDER BY anchor_seq ASC",
            (anchor_bound,),
        ).fetchall()
        invalid_anchors: set[int] = set()
        covered_nodes: set[int] = set()
        for anchor in anchor_rows:
            target_seq = anchor["comparison_seq"]
            # 指向目标链段之外（含位置非法）的锚点不在本次复核范围。
            if not isinstance(target_seq, int) or not (
                1 <= target_seq <= comparison_bound
            ):
                continue
            auditor_id = anchor["auditor_machine_id"]
            valid = True
            # 链位置须存在，且锚点冻结的链摘要须等于该节点保存的链摘要。
            target_chain = chain_map.get(target_seq)
            if target_chain is None or target_chain["chain_digest"] != anchor[
                "chain_digest"
            ]:
                valid = False
            ...
            if SLA_AUTH_MACHINE_PATTERN.fullmatch(auditor_id) is None:
                ...
                try:
                    public_bytes = bytes.fromhex(anchor["public_key"])
            ...
            if valid:
                message = (
                    f"{SLA_AUTH_CONTEXT}\nPOST\n/v1/audit-anchors\n"
                    f"{anchor['body_digest']}\n{anchor['request_time_ms']}\n"
                    f"{anchor['nonce']}\n{anchor['key_version']}\n"
                    f"{auditor_id}"
                ).encode("utf-8")
                try:
                    if not ed25519_verify(
                        public_bytes,
                        message,
                        bytes.fromhex(anchor["auth_signature"]),
                    ):
                        valid = False
                except ValueError:
                    valid = False
            if valid:
                ...
                invalid_anchors.add(anchor["anchor_seq"])
        uncovered_nodes = sorted(
            seq
            for seq in range(1, comparison_bound + 1)
            if seq not in covered_nodes
        )
        invalid_chain = sorted(invalid_chain_nodes)
        invalid_anchor = sorted(invalid_anchors)
    ...
    def _apply_audit_verification(
        ...
        comparison_seq = fields["comparisonSeq"]
        request_json = json.dumps(fields, sort_keys=True, separators=(",", ":"))
        standard_path = "/v1/audit-verifications"
        with closing(connect(self.server.database_path)) as database:
            database.execute("BEGIN IMMEDIATE")
            try:
                # 幂等判定先于授权与认证：同键更换正文或认证五段均冲突。
                record = database.execute(
                    ...
                    " auth_machine_id, auth_key_version, auth_request_time_ms,"
                    " auth_nonce, auth_signature"
                    " FROM audit_verification_idempotency_records WHERE key = ?",
                    (idempotency_key,),
                ).fetchone()
                if record is not None:
                    database.execute("ROLLBACK")
                ...
                try:
                    ...
                    database.execute("ROLLBACK")
                    return rejected.status, {"error": rejected.error}
                # 比较上界须指向已保存比较；超存储范围但格式合法
                # （参数无法绑定 SQLite 整数）同样按不存在处理。
                if comparison_seq <= INT64_MAX:
                    comparison_row = database.execute(
                        "SELECT 1 FROM audit_comparisons"
                        " WHERE comparison_seq = ?",
                        (comparison_seq,),
                    ).fetchone()
                ...
                if comparison_row is None:
                    database.execute("ROLLBACK")
                    return HTTPStatus.NOT_FOUND, {"error": "not_found"}
                # 创建事务以该比较为比较上界，并冻结全库最大锚点序号（空库为 0）。
                anchor_bound = database.execute(
                    "SELECT COALESCE(MAX(anchor_seq), 0) AS anchor_bound"
                    " FROM audit_anchors"
                ).fetchone()["anchor_bound"]
                (
                    chain_consistent,
                    fully_covered,
                    invalid_chain,
                    invalid_anchors,
                    uncovered_nodes,
                ) = self._compute_audit_verification(
                    database, comparison_seq, anchor_bound
                )
                # 摘要取两个上界、两项结论及三组异常数组的紧凑 JSON 字节
                # （无尾换行）之 SHA-256 小写十六进制。
                summary = {
                    "comparisonSeqBound": comparison_seq,
                    "anchorSeqBound": anchor_bound,
                    "chainConsistent": chain_consistent,
                    "fullyCovered": fully_covered,
                    "invalidChainNodes": invalid_chain,
                    "invalidAnchors": invalid_anchors,
                ...
                database.execute(
                    "INSERT INTO audit_verification_idempotency_records"
                    "(key, request_json, status, response_json,"
                    " auth_machine_id, auth_key_version, auth_request_time_ms,"
                    " auth_nonce, auth_signature)"
                    " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def _audit_verification_collection_page(
        self, database: Any, cut: int, last_seq: int, limit: int
    ) -> tuple[list[dict[str, Any]], str | None]:
        rows = database.execute(
            "SELECT verification_seq, response_json FROM audit_verifications"
    ...
    def _get_audit_verifications(self, query: str) -> None:
        # 验证报告集合分页沿用审计锚点列表的 limit 与 cursor=cut:lastSeq。
        parsed = self._parse_evaluation_query(
            query, AUDIT_VERIFICATIONS_QUERY_PARAMS
        )
        if parsed is None:
            self._json(HTTPStatus.BAD_REQUEST, {"error": "invalid_request"})
            return
        ...
        with closing(connect(self.server.database_path)) as database:
            ...
            try:
                ...
                verifications, next_cursor = (
                    self._audit_verification_collection_page(
                        database, cut, last_seq, limit
                    )
                )
        ...
        with closing(connect(self.server.database_path)) as database:
            ...
            try:
                ...
                if report is None:
                    database.execute("ROLLBACK")
                    return HTTPStatus.NOT_FOUND, {"error": "not_found"}
                # 待签文本以 audit-proof-v1 开头，逐行连接报告序号、原始响应
                # 摘要、报告摘要、比较上界与锚点上界，末尾无换行；原始响应摘要
                # 取创建报告时保存的完整紧凑 UTF-8 无尾换行字节之 SHA-256。
    ...
    def _get_audit_proofs(self, query: str) -> None:
        # 证明集合分页沿用验证报告列表的 limit 与 cursor=cut:lastSeq。
