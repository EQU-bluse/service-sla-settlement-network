...
- `POST /v1/slas/{id}/evaluations`：对 active 状态的 SLA 在给定区间上做一次结算评估。请求须携带 `Idempotency-Key` 头（格式同上），JSON 请求体恰含且不得重复 `from`、`to`，均为非布尔整数。仅当 SLA 为 `active` 且 `start*1000<=from<to<=end*1000` 时接受。事务起点取全库最大 `commit_seq` 为 `cut`（空库为 `0`），只聚合该 SLA 内 `commit_seq<=cut` 且 `timestamp∈[from,to)` 的事件；`count`、`latencySum`、`maxLatency`、`violations` 定义与遥测查询的 `summary` 相同（`violations` 为延迟严格大于 SLA 快照 `maxLatencyMs` 的事件数）。`outcome` 在 `count=0` 时为 `insufficient`，否则 `violations=0` 时为 `fulfilled`，其余为 `breached`。首次返回 `201`，正文键序为 `from,to,cut,count,latencySum,maxLatency,violations,outcome`；`outcome` 为字符串，`maxLatency` 空时为 `null`，其余字段为非负整数。首次成功在同一事务内原子分配全库共享的持久化评估序号 `evaluationSeq`（空库首条为 `1`，否则取提交前全库最大序号加一）与 `createdAt`（事务时刻 UTC 毫秒），二者为非负整数且不随响应重放；失败请求与同键重放均不分配序号，重放返回首次状态码与响应字节。判定顺序为头、体、幂等记录、SLA、状态/区间：头或体非法返回 `400`/`invalid_request`；同键异 id 或异体返回 `409`/`conflict`；SLA 不存在返回 `404`/`not_found`；非 active 或区间越界（含 `from>=to`）返回 `409`/`conflict`。快照与幂等结果在同一事务原子持久化，后续遥测不得改变；同键同请求并发或重启后均重放首次状态码与响应字节，失败不留记录；跨 SLA 并发评估不得产生重复序号。旧库迁移时，在单事务内按 `sla_evaluation_idempotency_records.rowid` 升序将既有评估稠密赋号 `1..N`、`createdAt` 置 `0` 并写入一次性标记；快照、幂等数据与重放字节保持不变，重启后不再重排。
- `GET /v1/slas/{id}/evaluations`：分页读取 SLA 评估。查询参数仅允许且每项限单值 `limit`、`cursor`；`limit` 为无前导零十进制 `1..100`，缺省 `50`；缺项、未知/重复参数、格式或范围非法均为 `400`/`invalid_request`。参数校验先验于 SLA 查找；SLA 不存在（含非法 id）为 `404`/`not_found`。无 `cursor` 时，`cut` 取同一读事务起点的全库最大 `evaluationSeq`（空库为 `0`）；按序号升序返回该 SLA 内 `0<evaluationSeq<=cut` 的评估。`cursor=cut:lastSeq`（两段均为无前导零十进制）：续页仅含序号严格大于 `lastSeq` 且不大于 `cut` 的条目；`cut` 大于当前全库最大序号、或 `lastSeq` 在该 SLA 内且不大于 `cut` 的锚点不存在，均为 `400`/`invalid_request`。成功为 `200`，正文键序 `evaluations,nextCursor`；元素键序 `evaluationSeq,from,to,cut,count,latencySum,maxLatency,violations,outcome,createdAt`，前九项类型与取值同 POST 首次响应（`createdAt` 为非负整数）。页长不超过 `limit`；有后页时 `nextCursor` 为 `cut:末序号`，否则为 `null`。`cut` 由游标携带并跨重启稳定；读取期间的并发新增不影响续页（全新首页才取新 `cut`）。
