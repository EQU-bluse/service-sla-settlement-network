...
- `POST /v1/slas/{id}/telemetry`：为 active 状态的 SLA 写入一条遥测事件。请求须携带 `Idempotency-Key` 头（格式同上），JSON 请求体仅含且不得重复 `eventId`（`[a-z0-9-]{1,64}`）、`timestamp`（非布尔整数 `0..2147483647999`，UTC Unix 毫秒）、`latencyMs`（非布尔整数 `0..2147483647`）、`digest`（`[0-9a-f]{64}`）。`digest` 等于 UTF-8 字符串 `id\neventId\ntimestamp\nlatencyMs\nmachineId` 的 SHA-256（整数为无前导零十进制，`machineId` 取 SLA 快照）。仅当 SLA 为 `active` 且 `start*1000<=timestamp<end*1000` 时接受。首次原子写入事件及幂等结果，返回 `201`，正文仅 `{"eventId":请求值}`。判定顺序为头、体、幂等记录、SLA、状态/时间、摘要、eventId：头或体非法返回 `400`/`invalid_request`；同键异 id 或异体返回 `409`/`conflict`；SLA 不存在返回 `404`/`not_found`；非 active、时间越界或摘要错误返回 `409`/`conflict`；异键重复 eventId 返回 `409`/`event_exists`。同键同请求并发或重启后重放首次状态码与响应字节；事件与幂等结果原子持久化，失败不留记录；异键同 eventId 并发仅一项成功。每条事件在提交时分配全局单调递增的提交序号，序号跨重启稳定。
- `GET /v1/slas/{id}/telemetry`：分页读取 SLA 的遥测事件。查询参数仅允许单值 `from`、`to`、`limit`、`cursor`；`from`、`to` 必填，为无前导零十进制且 `0<=from<to<=2147483648000`；`limit` 缺省 `50`，范围 `1..100`。无 `cursor` 时 `cut` 取事务起点的最大提交序号（无事件为 `0`）；仅读取序号 `<=cut` 且 `timestamp∈[from,to)` 的事件，按 `timestamp,eventId` 升序。`cursor` 格式为 `cut:lastTimestamp:lastEventId`（数字无前导零），其末事件须属于同一 SLA、`cut` 与请求区间，续页从该事件之后开始。缺项、未知或重复参数、格式/范围/cursor 关联非法均返回 `400`/`invalid_request`；SLA 不存在返回 `404`/`not_found`。`200` 正文键序为 `events,summary,nextCursor`；`events` 为对象数组，元素键序为 `eventId,timestamp,latencyMs,digest`；`summary` 键序为 `count,latencySum,maxLatency,violations`，依次为全 `cut` 区间事件数、延迟和、最大延迟（空区间为 `null`）、延迟大于快照 `maxLatencyMs` 的事件数，非 `null` 值均为非负整数；`nextCursor` 有后页时取末事件游标，否则为 `null`。序号与 `cut` 跨重启稳定，并发新增不影响续页与汇总。
