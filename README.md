...
- `POST /v1/slas`：基于模板创建 SLA。请求须携带 `Idempotency-Key` 头（格式同上），JSON 请求体仅含且不得重复 `id`、`templateId`（均为 `[a-z0-9-]{1,64}`）、`consumerId`（`[0-9a-f]{64}`）、`start`、`end`（非布尔整数 `0..2147483647`，且 `start < end`，UTC Unix 秒）。首次创建须保证模板存在、其机器当前能力版本仍等于模板绑定版本且消费者已登记；请求与模板快照（`machineId`、`capabilityVersion`、`priceMicros`、`maxLatencyMs`）在同一原子事务中保存，初态 `pending`，返回 `201`，正文仅 `{"id":请求id}`。判定顺序为头、体、幂等记录、模板与版本、消费者、id：头或体非法返回 `400`/`invalid_request`；同键异请求、模板缺失或版本失效返回 `409`/`conflict`；消费者不存在返回 `404`/`not_found`；异键同 id 返回 `409`/`sla_exists`。
- `GET /v1/slas/{id}`：返回 `200`，正文键序为 `id,templateId,machineId,consumerId,capabilityVersion,priceMicros,maxLatencyMs,start,end,state`，同名值取自创建时请求或模板快照，`state` 为 `pending`。无记录（含非法 id）返回 `404`/`not_found`。
