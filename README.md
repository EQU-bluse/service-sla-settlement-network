...
- `POST /v1/slas/{id}/telemetry`：为 active 状态的 SLA 提交一条遥测事件。请求须携带 `Idempotency-Key` 头（格式同上），JSON 请求体仅含且不得重复 `eventId`（`[a-z0-9-]{1,64}`）、`timestamp`（非布尔整数 `0..2147483647999`，毫秒）、`latencyMs`（非布尔整数 `0..2147483647`）、`digest`（64 位小写十六进制）。`digest` 等于 UTF-8 字符串 `id\neventId\ntimestamp\nlatencyMs\nmachineId` 的 SHA-256，其中整数为无前导零十进制，`machineId` 取 SLA 快照。仅当 SLA 为 `active` 且 `start*1000<=timestamp<end*1000` 时接收。首次原子写入事件与幂等结果，返回 `201`，正文仅 `{"eventId":请求值}`。判定顺序为头、体、幂等记录、SLA、状态/时间、摘要、eventId：头或体非法返回 `400`/`invalid_request`；同键异 id 或异体返回 `409`/`conflict`；SLA 不存在返回 `404`/`not_found`；非 active、越出时间窗或摘要错误返回 `409`/`conflict`；异键重复 eventId 返回 `409`/`event_exists`。同键并发或重启后重放返回首次状态码与响应字节；事件与幂等结果原子持久化，失败无副作用；异键同 eventId 仅一项成功。
