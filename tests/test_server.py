    ...
    def test_issue_evidence_scope_persisted_and_listed(self) -> None:
        ...
        self.assertEqual(
            (item["operation"], item["capabilityVersion"], item["disputeId"],
             item["evidenceSeq"]),
            ("evidence.write", None, "dispute-1", None),
        )
    ...
    def test_consumed_event_records_resource_and_digest(self) -> None:
        ...
        self.assertEqual(
            ...
            ["eventSeq", "delegationId", "operation", "capabilityVersion",
             "resource", "requestDigest", "createdAt", "evidenceSeq"],
        )
...
class AdjudicationProposalTests(_EvidenceScenario, unittest.TestCase):
    # 双边裁决提案：首方冻结为 pending，双方同快照同决定原子裁决；
    # 快照或决定不同保存为 disagreement；POST/GET 的校验沿用证据快照入口。
    def snapshot_body(self, actor: str | None = None) -> dict[str, object]:
        return {"actorId": actor if actor is not None else self.consumer_id}
    ...
    def proposal_body(
        ...
        *,
        ...
        return {
            ...
            "decision": decision,
            "reasonDigest": reason_digest if reason_digest is not None else "cd" * 32,
        }

    def create_snapshot(self, *, actor: str | None = None, key: str = "snap-1") -> int:
        status, body = self.post_json(
            "/v1/disputes/dispute-1/evidence-snapshots",
            self.snapshot_body(actor if actor is not None else self.consumer_id),
            key,
        )
        self.assertEqual(status, 201, body)
    ...
    def get_proposals(
        ...
        *,
        ...
        # 提案审计读取为 GET 且无正文；标准路径不含查询串，摘要按空字节计算。
        request = Request(self.url(path), data=b"", method="GET")
        if not omit_auth:
            if nonce is None:
    ...
    def proposals_path(self, dispute_id: str = "dispute-1") -> str:
        return f"/v1/disputes/{dispute_id}/adjudication-proposals"
    ...
    def dispute_events(self, dispute_id: str = "dispute-1") -> list[tuple]:
        connection = sqlite3.connect(self.server.database_path)
        try:
            return connection.execute(
                "SELECT type FROM dispute_events WHERE dispute_id = ?"
                " ORDER BY event_seq",
                (dispute_id,),
            ).fetchall()
        finally:
    ...
    def test_first_proposal_freezes_snapshot_as_pending(self) -> None:
        ...
        self.assertIsInstance(payload["createdAt"], int)
        self.assertGreaterEqual(payload["createdAt"], 0)
        self.assertEqual(body, b'{"proposalSeq":1,"result":"pending","decision"'
                             b':"refund","amount":null,"createdAt":'
                             + str(payload["createdAt"]).encode() + b"}")
        # 首方提案不裁决：争议仍 open，无新生命周期事件与账本分录。
        status, dispute = self.get_json("/v1/disputes/dispute-1")
        self.assertEqual(dispute["state"], "open")
        self.assertEqual(self.dispute_events(), [("opened",)])

    def test_matching_refund_resolves_atomically(self) -> None:
        self.create_snapshot()
        status, first = self.post_proposal(key="prop-c")
        self.assertEqual(status, 201)
        self.assertEqual(json.loads(first)["result"], "pending")
        status, body = self.post_proposal(
            payload=self.proposal_body(
                actor=self.machine_id, decision="refund", reason_digest="de" * 32
            ),
            key="prop-p",
        )
        self.assertEqual(status, 201)
        ...
        # 双方各一笔 dispute_refund 分录，保留结算关联。
        ...
        self.assertEqual(self.dispute_events(), [("opened",), ("refunded",)])

    def test_matching_release_resolves_without_ledger_entries(self) -> None:
        self.create_snapshot()
        status, _ = self.post_proposal(
            payload=self.proposal_body(decision="release"), key="prop-c"
        )
        self.assertEqual(status, 201)
        status, body = self.post_proposal(
            payload=self.proposal_body(actor=self.machine_id, decision="release"),
            key="prop-p",
        )
        self.assertEqual(status, 201)
        payload = json.loads(body)
        ...
        self.assertFalse(
            any(e["kind"] == "dispute_refund" for e in ledger["entries"])
        )
        self.assertEqual(self.dispute_events(), [("opened",), ("released",)])

    def test_different_decision_saves_disagreement_and_keeps_open(self) -> None:
        self.create_snapshot()
        status, _ = self.post_proposal(
            payload=self.proposal_body(decision="refund"), key="prop-c"
        )
        self.assertEqual(status, 201)
        status, body = self.post_proposal(
            payload=self.proposal_body(actor=self.machine_id, decision="release"),
    ...
    def test_different_snapshot_saves_disagreement(self) -> None:
        first_seq = self.create_snapshot(key="snap-1")
        status, second_body = self.post_json(
            "/v1/disputes/dispute-1/evidence-snapshots",
            self.snapshot_body(self.machine_id), "snap-2",
        )
        self.assertEqual(status, 201)
        second_seq = json.loads(second_body)["snapshotSeq"]
        status, _ = self.post_proposal(
            payload=self.proposal_body(snapshot_seq=first_seq, decision="release"),
            key="prop-c",
        )
        self.assertEqual(status, 201)
        status, body = self.post_proposal(
            payload=self.proposal_body(
                actor=self.machine_id, snapshot_seq=second_seq, decision="release",
            ),
            key="prop-p",
        )
        self.assertEqual(status, 201)
    ...
    def test_same_party_second_proposal_conflicts_without_sequence_gap(self) -> None:
        self.create_snapshot()
        status, first = self.post_proposal(key="prop-c")
        self.assertEqual(status, 201)
        self.assertEqual(json.loads(first)["proposalSeq"], 1)
        status, body = self.post_proposal(
            payload=self.proposal_body(decision="release"), key="prop-c-other"
        )
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "proposal_exists"})
        # 另一参与方仍可提交并正常裁决；失败不推进序号，成功提案序号紧接为 2。
        status, body = self.post_proposal(
            payload=self.proposal_body(actor=self.machine_id), key="prop-p"
        )
        self.assertEqual(status, 201)
        self.assertEqual(json.loads(body)["proposalSeq"], 2)
        self.assertEqual(json.loads(body)["result"], "resolved")

    def test_refund_insufficient_funds_saves_nothing(self) -> None:
        self.create_snapshot()
        status, _ = self.post_proposal(key="prop-c")
        self.assertEqual(status, 201)
        # 收款方（机器）总余额被提空（争议冻结仍登记 open），退款必然不足。
        connection = sqlite3.connect(self.server.database_path)
        try:
            connection.execute(
            ...
            connection.close()
        raw = json.dumps(
            self.proposal_body(actor=self.machine_id, decision="refund")
        ).encode()
        fixed_nonce = "nonce-prop-insufficient-000001"
        status, body = self.post_proposal_raw(
            self.proposals_path(), raw, "prop-p-fail",
            seed=PRODUCER_SEED, actor=self.machine_id, nonce=fixed_nonce,
        )
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "insufficient_funds"})
        # 第二份提案、裁决事件、幂等结果均不保存；争议保持 open 与冻结。
        connection = sqlite3.connect(self.server.database_path)
        try:
            count = connection.execute(
                "SELECT COUNT(*) AS c FROM dispute_adjudication_proposals"
            ).fetchone()[0]
            idem = connection.execute(
                "SELECT COUNT(*) AS c FROM"
                " dispute_adjudication_proposal_idempotency_records"
            ).fetchone()[0]
            state = connection.execute(
                "SELECT state FROM disputes WHERE id = 'dispute-1'"
            ).fetchone()[0]
        finally:
            connection.close()
        self.assertEqual(count, 1)
        self.assertEqual(idem, 1)
        self.assertEqual(state, "open")
        self.assertEqual(self.dispute_events(), [("opened",)])
        # 失败不消费随机数：同随机数重试仍是余额不足而非 replay_detected。
        status, body = self.post_proposal_raw(
            self.proposals_path(), raw, "prop-p-fail-2",
            seed=PRODUCER_SEED, actor=self.machine_id, nonce=fixed_nonce,
        )
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "insufficient_funds"})
        # 原幂等键未占用：补足余额后同键重试成功并裁决。
        status, _ = self.post_json(
            f"/v1/funds/{self.machine_id}",
            {"amountMicros": 5000, "reference": "ref-fund-prop"},
            "fund-prop",
        )
        self.assertEqual(status, 201)
        status, body = self.post_proposal_raw(
            self.proposals_path(), raw, "prop-p-fail",
            seed=PRODUCER_SEED, actor=self.machine_id, nonce="nonce-prop-retry-001",
        )
        self.assertEqual(status, 201, body)
        self.assertEqual(json.loads(body)["result"], "resolved")

    def test_resolved_dispute_rejects_proposals(self) -> None:
        self.create_snapshot()
        status, _ = self.post_proposal(key="prop-c")
        self.assertEqual(status, 201)
        status, _ = self.post_json(
        ...
        self.assertEqual(status, 200)
        status, body = self.post_proposal(
            payload=self.proposal_body(actor=self.machine_id), key="prop-late"
        )
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "already_resolved"})
    ...
    def test_missing_dispute_is_404(self) -> None:
        self.create_snapshot()
        status, body = self.post_proposal(
            dispute_id="dispute-ghost", key="prop-ghost"
        )
        # 快照不属于该争议：争议存在性先判，争议本身缺失即 404。
        self.assertEqual(status, 404)
        self.assertEqual(json.loads(body), {"error": "not_found"})

    def test_missing_or_cross_dispute_snapshot_is_404(self) -> None:
        self.create_snapshot()
        # 快照序号不存在。
        status, body = self.post_proposal(
            payload=self.proposal_body(snapshot_seq=99), key="prop-no-snap"
        )
        self.assertEqual(status, 404)
        self.assertEqual(json.loads(body), {"error": "not_found"})
        # 跨争议引用：dispute-1 的快照用在 dispute-2 上。
        self.create_second_dispute()
        status, body = self.post_proposal(
            dispute_id="dispute-2",
            payload=self.proposal_body(snapshot_seq=1, decision="release"),
            key="prop-cross",
        )
        self.assertEqual(status, 404)
        self.assertEqual(json.loads(body), {"error": "not_found"})
    ...
    def test_actor_must_be_party_and_match_auth_machine(self) -> None:
        ...
        self.assertEqual(status, 201)
        stranger = machine_id(PUBLIC_KEY_C)
        raw = json.dumps(self.proposal_body(actor=stranger)).encode()
        status, body = self.post_proposal_raw(
            self.proposals_path(), raw, "prop-stranger",
            seed=PUBLIC_KEY_SEED_C, actor=stranger,
        )
        self.assertEqual(status, 403)
        self.assertEqual(json.loads(body), {"error": "forbidden"})
        # 正文 actorId 与认证机器不一致：生产方密钥签消费方身份。
        raw = json.dumps(self.proposal_body()).encode()
        status, body = self.post_proposal_raw(
            self.proposals_path(), raw, "prop-mismatch",
            seed=PRODUCER_SEED, actor=self.machine_id,
        )
        self.assertEqual(status, 403)
        self.assertEqual(json.loads(body), {"error": "forbidden"})
    ...
    def test_invalid_requests(self) -> None:
        self.create_snapshot()
        valid = self.proposal_body()
        cases = [
            b"",
            b"{not json",
            json.dumps({}).encode(),
            json.dumps({"actorId": valid["actorId"], "snapshotSeq": 1,
                        "decision": "refund"}).encode(),
            json.dumps({**valid, "extra": 1}).encode(),
            json.dumps({**valid, "actorId": "zz"}).encode(),
            json.dumps({**valid, "actorId": valid["actorId"].upper()}).encode(),
            json.dumps({**valid, "actorId": 1}).encode(),
            json.dumps({**valid, "snapshotSeq": 0}).encode(),
            json.dumps({**valid, "snapshotSeq": True}).encode(),
            json.dumps({**valid, "snapshotSeq": "1"}).encode(),
            json.dumps({**valid, "decision": "void"}).encode(),
            json.dumps({**valid, "reasonDigest": "ZZ" * 32}).encode(),
            json.dumps({**valid, "reasonDigest": "abc"}).encode(),
            json.dumps({**valid, "reasonDigest": 1}).encode(),
        ]
        for index, bad in enumerate(cases):
            status, body = self.post_proposal_raw(
                self.proposals_path(), bad, f"bad-{index}",
                seed=PUBLIC_KEY_SEED_B, actor=self.consumer_id,
            )
            self.assertEqual(status, 400, bad)
            self.assertEqual(json.loads(body), {"error": "invalid_request"})
        ...
        status, _ = self.post_proposal_raw(
            self.proposals_path(), json.dumps(valid).encode(), None,
            seed=PUBLIC_KEY_SEED_B, actor=self.consumer_id,
        )
        self.assertEqual(status, 400)
        # 查询参数非法先于争议查询。
        status, _ = self.post_proposal_raw(
            self.proposals_path() + "?x=1", json.dumps(valid).encode(),
            "prop-query", seed=PUBLIC_KEY_SEED_B, actor=self.consumer_id,
        )
        self.assertEqual(status, 400)
        # 缺少认证头。
        status, _ = self.post_proposal_raw(
            self.proposals_path(), json.dumps(valid).encode(), "prop-no-auth",
        )
        self.assertEqual(status, 400)
        # 携带代理头同样非法。
        status, _ = self.post_proposal_raw(
            self.proposals_path(), json.dumps(valid).encode(),
            "prop-delegation",
            delegation="del-1;0;0;nonce-delegation-00000000;" + "ab" * 64,
        )
        self.assertEqual(status, 400)
    ...
    def test_stale_request_is_401(self) -> None:
        ...
        stale = int(time.time() * 1000) - 301_000
        status, body = self.post_proposal_raw(
            self.proposals_path(), raw, "prop-stale",
            seed=PUBLIC_KEY_SEED_B, actor=self.consumer_id,
            request_time_ms=stale,
        )
        self.assertEqual(status, 401)
    ...
    def test_nonce_replay_detected(self) -> None:
        self.create_snapshot()
        raw = json.dumps(self.proposal_body()).encode()
        fixed = "nonce-prop-fixed-0000000001"
        status, _ = self.post_proposal_raw(
            self.proposals_path(), raw, "prop-nonce-1",
            seed=PUBLIC_KEY_SEED_B, actor=self.consumer_id, nonce=fixed,
        )
        self.assertEqual(status, 201)
        status, body = self.post_proposal_raw(
            self.proposals_path(),
            json.dumps(self.proposal_body(decision="release")).encode(),
            "prop-nonce-2",
            seed=PUBLIC_KEY_SEED_B, actor=self.consumer_id, nonce=fixed,
        )
        self.assertEqual(status, 409)
    ...
    def test_same_key_conflict_and_replay_bytes(self) -> None:
        self.create_snapshot()
        status, first = self.post_proposal(key="prop-1")
        self.assertEqual(status, 201)
        # 同键重放原字节。
        status, replay = self.post_proposal(key="prop-1")
        self.assertEqual(status, 201)
        self.assertEqual(replay, first)
        # 同键异正文冲突，且先于争议查询。
        status, body = self.post_proposal(
            payload=self.proposal_body(decision="release"), key="prop-1"
        )
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "conflict"})
        status, body = self.post_proposal(
            dispute_id="dispute-other", key="prop-1"
        )
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "conflict"})
        # 同键异认证五段冲突。
        raw = json.dumps(self.proposal_body()).encode()
        status, body = self.post_proposal_raw(
            self.proposals_path(), raw, "prop-1",
            seed=PUBLIC_KEY_SEED_B, actor=self.consumer_id,
            nonce="nonce-prop-replay-other-0001",
        )
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "conflict"})
    ...
    def test_replay_survives_restart(self) -> None:
        ...
        self.assertEqual(status, 201)
        self.assertEqual(replay, first)
        status, body = self.get_proposals(self.proposals_path())
        self.assertEqual(status, 200)
        self.assertEqual(
            json.loads(body)["proposals"][0]["proposalSeq"],
            json.loads(first)["proposalSeq"],
        )

    def test_global_sequence_spans_disputes_and_failures_do_not_advance(self) -> None:
        self.create_snapshot(key="snap-d1")
        status, body = self.post_proposal(key="prop-d1")
        self.assertEqual(status, 201)
        self.assertEqual(json.loads(body)["proposalSeq"], 1)
        self.create_second_dispute()
        status, snap2_body = self.post_json(
            "/v1/disputes/dispute-2/evidence-snapshots",
            {"actorId": self.consumer_id}, "snap-d2",
        )
        self.assertEqual(status, 201)
        snap2_seq = json.loads(snap2_body)["snapshotSeq"]
        status, body = self.post_proposal(
            dispute_id="dispute-2",
            payload=self.proposal_body(
                snapshot_seq=snap2_seq, decision="release"
            ),
            key="prop-d2",
        )
        self.assertEqual(status, 201)
        ...
        # 失败请求不推进序号。
        status, _ = self.post_proposal(
            payload=self.proposal_body(snapshot_seq=99), key="prop-missing-snap"
        )
        self.assertEqual(status, 404)
        status, body = self.post_proposal(
            payload=self.proposal_body(
                actor=self.machine_id, decision="release"
            ),
            key="prop-d1-p",
        )
        self.assertEqual(status, 201)
        self.assertEqual(json.loads(body)["proposalSeq"], 3)

    def test_concurrent_matching_proposals_adjudicate_once(self) -> None:
        self.create_snapshot()
        barrier = threading.Barrier(2)
        results: list[dict] = []
        lock = threading.Lock()
        ...
        def submit(seed: bytes, actor: str, key: str) -> None:
            barrier.wait()
            status, body = self.post_proposal(
                payload=self.proposal_body(actor=actor, decision="refund"),
                key=key,
            )
            with lock:
                results.append({"status": status, **json.loads(body)})

        threads = [
            threading.Thread(
                target=submit,
                args=(PUBLIC_KEY_SEED_B, self.consumer_id, "prop-race-c"),
            ),
            threading.Thread(
                target=submit,
                args=(PRODUCER_SEED, self.machine_id, "prop-race-p"),
            ),
        ]
        for thread in threads:
            thread.start()
        for thread in threads:
            thread.join(timeout=10)
        self.assertEqual(sorted(r["status"] for r in results), [201, 201])
        self.assertEqual(sorted(r["result"] for r in results), ["pending", "resolved"])
        self.assertEqual(
            sorted(r["proposalSeq"] for r in results), [1, 2]
        )
        # 资金只移动一次，裁决事件仅一条。
        status, ledger = self.get_json(
            f"/v1/accounts/{self.machine_id}/ledger"
    ...
    def test_get_collection_empty(self) -> None:
        ...
        self.assertEqual(status, 200)
        self.assertEqual(
            json.loads(body), {"proposals": [], "nextCursor": None}
        )

    def test_get_collection_returns_frozen_digest_and_results(self) -> None:
        self.create_snapshot()
        first_status, first_body = self.post_proposal(
            payload=self.proposal_body(decision="refund"), key="prop-c"
        )
        self.assertEqual(first_status, 201)
        second_status, _ = self.post_proposal(
            payload=self.proposal_body(actor=self.machine_id), key="prop-p"
        )
        ...
        self.assertEqual(status, 200)
        payload = json.loads(body)
        self.assertEqual(list(payload), ["proposals", "nextCursor"])
        proposals = payload["proposals"]
        self.assertEqual(
            [list(item) for item in proposals],
            [
                [
                    "proposalSeq", "actorId", "snapshotSeq", "snapshotDigest",
                    "decision", "reasonDigest", "result", "amount", "createdAt",
                ],
                [
                    "proposalSeq", "actorId", "snapshotSeq", "snapshotDigest",
                    "decision", "reasonDigest", "result", "amount", "createdAt",
        ...
        self.assertEqual([item["proposalSeq"] for item in proposals], [1, 2])
        first, second = proposals
        self.assertEqual(first["actorId"], self.consumer_id)
        self.assertEqual(first["snapshotSeq"], 1)
        self.assertEqual(first["decision"], "refund")
        self.assertEqual(first["reasonDigest"], "cd" * 32)
        self.assertEqual(first["result"], "pending")
        self.assertIsNone(first["amount"])
        self.assertEqual(second["actorId"], self.machine_id)
        self.assertEqual(second["result"], "resolved")
        self.assertEqual(second["amount"], 1000)
        # 双方引用同一快照：冻结摘要等于快照摘要。
        status, snapshot_body = self.get_snapshot(
            "/v1/disputes/dispute-1/evidence-snapshots/1",
            seed=PRODUCER_SEED, actor=self.machine_id,
        )
        self.assertEqual(status, 200)
        snapshot_digest = json.loads(snapshot_body)["digest"]
        self.assertEqual(first["snapshotDigest"], snapshot_digest)
        self.assertEqual(second["snapshotDigest"], snapshot_digest)
        self.assertIsNone(payload["nextCursor"])

    def test_get_collection_readable_after_resolution(self) -> None:
        self.create_snapshot()
        status, _ = self.post_proposal(
            payload=self.proposal_body(decision="release"), key="prop-c"
        )
        ...
        status, body = self.get_proposals(
            self.proposals_path(), seed=PRODUCER_SEED, actor=self.machine_id
        )
        self.assertEqual(status, 200)
        self.assertEqual(
            [item["result"] for item in json.loads(body)["proposals"]],
            ["pending", "resolved"],
        )

    def test_get_collection_pagination_and_cursor_isolation(self) -> None:
        self.create_snapshot()
        self.post_proposal(key="prop-page-1")
        self.post_proposal(
            payload=self.proposal_body(
                actor=self.machine_id, decision="refund",
                reason_digest="ef" * 32,
            ),
            key="prop-page-2",
        )
        status, first_page = self.get_proposals(
            self.proposals_path() + "?limit=1"
        )
        self.assertEqual(status, 200)
        first_payload = json.loads(first_page)
        self.assertEqual(
            [item["proposalSeq"] for item in first_payload["proposals"]], [1]
        )
        self.assertEqual(first_payload["nextCursor"], "2:1")
        # 裁决后新增的提案不进入旧 cut 续页（本争议已裁决，改用跨争议新增验证）。
        self.create_second_dispute()
        status, snap2_body = self.post_json(
            "/v1/disputes/dispute-2/evidence-snapshots",
            {"actorId": self.consumer_id}, "snap-other",
        )
        self.assertEqual(status, 201)
        snap2_seq = json.loads(snap2_body)["snapshotSeq"]
        self.assertEqual(
        ...
        status, second_page = self.get_proposals(
            self.proposals_path() + "?limit=1"
            f"&cursor={first_payload['nextCursor']}"
        )
        self.assertEqual(status, 200)
        second_payload = json.loads(second_page)
        self.assertEqual(
            [item["proposalSeq"] for item in second_payload["proposals"]], [2]
        )
        self.assertIsNone(second_payload["nextCursor"])

    def test_get_collection_errors_and_nonce_consumption(self) -> None:
        self.create_snapshot()
        self.post_proposal(key="prop-1")
        for query in (
            "limit=0", "limit=101", "limit=01", "limit=1&limit=2",
            "unknown=1", "cursor=1", "cursor=x:1", "cursor=1:y",
        ):
            status, body = self.get_proposals(
                self.proposals_path() + f"?{query}"
            )
            self.assertEqual(status, 400, query)
            self.assertEqual(json.loads(body), {"error": "invalid_request"})
        # 参数错误先于争议查询。
        status, _ = self.get_proposals(
            self.proposals_path("dispute-missing") + "?limit=0"
        )
        self.assertEqual(status, 400)
        # 争议不存在 404；非参与方 403。
        status, body = self.get_proposals(self.proposals_path("dispute-missing"))
        self.assertEqual(status, 404)
        self.assertEqual(json.loads(body), {"error": "not_found"})
        status, _ = self.post_json(
            "/v1/machines", {"publicKey": PUBLIC_KEY_C}, "register-3"
        ...
        status, body = self.get_proposals(
            self.proposals_path() + "?cursor=99:1"
        )
        self.assertEqual(status, 400)
        self.assertEqual(json.loads(body), {"error": "invalid_request"})
        status, body = self.get_proposals(
            self.proposals_path() + "?cursor=2:2"
        )
        self.assertEqual(status, 400)
        self.assertEqual(json.loads(body), {"error": "invalid_request"})
        # 成功读取消费随机数；失败（cut 超前）不消费。
        fixed = "nonce-prop-get-000000000001"
        status, _ = self.get_proposals(
            self.proposals_path() + "?cursor=99:1", nonce=fixed
        )
        self.assertEqual(status, 400)
        status, body = self.get_proposals(
            self.proposals_path() + "?cursor=99:1", nonce=fixed
        ...
        self.assertEqual(status, 400)
        status, body = self.get_proposals(self.proposals_path(), nonce=fixed)
        self.assertEqual(status, 200, body)
        status, body = self.get_proposals(self.proposals_path(), nonce=fixed)
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "replay_detected"})
        ...
        status, _ = self.get_proposals(
            self.proposals_path("dispute-missing"), omit_auth=True
        )
        self.assertEqual(status, 400)
        request = Request(
            self.url(self.proposals_path()), data=b"", method="GET"
        )
        request.add_header(
            "SLA-Delegation", "del-1;0;0;nonce-delegation-00000001;" + "ab" * 64
        )
...
class AuditVerificationTests(_EvidenceScenario, unittest.TestCase):
    ...
    def request_raw(
        ...
        *,
        ...
        if key is None and nonce is None and auth_header is None:
            nonce = f"nonce-vf-get-{time.time_ns()}"
        if delegation is not None:
            request.add_header("SLA-Delegation", delegation)
        elif not omit_auth:
            request.add_header(
    ...
    def expected_digest(payload: dict[str, Any]) -> str:
        summary = {
            "comparisonSeqBound": payload["comparisonSeqBound"],
            "anchorSeqBound": payload["anchorSeqBound"],
            "chainConsistent": payload["chainConsistent"],
            "fullyCovered": payload["fullyCovered"],
            "invalidChainNodes": payload["invalidChainNodes"],
            "invalidAnchors": payload["invalidAnchors"],
    ...
    def test_clean_verification_full_coverage(self) -> None:
        self.create_comparison(1, "vf-cl-1")
        self.create_comparison(2, "vf-cl-2")
        self.create_anchor(1, "vf-cl-a1")
        self.create_anchor(2, "vf-cl-a2")
        status, response = self.post_verification(2, "vf-clean")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertEqual(
            list(payload),
        ...
        self.assertEqual(payload["comparisonSeqBound"], 2)
        self.assertEqual(payload["anchorSeqBound"], 2)
        self.assertEqual(payload["createdBy"], self.auditor_id)
        self.assertIsInstance(payload["createdAt"], int)
        self.assertGreaterEqual(payload["createdAt"], 0)
    ...
    def test_no_anchors_means_uncovered_but_chain_consistent(self) -> None:
        self.create_comparison(1, "vf-ua-1")
        status, response = self.post_verification(1, "vf-uncovered")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertEqual(payload["verificationSeq"], 1)
    ...
    def test_partial_bound_and_frozen_anchor_bound(self) -> None:
        self.create_comparison(1, "vf-pb-1")
        self.create_comparison(2, "vf-pb-2")
        self.create_anchor(1, "vf-pb-a1")
        status, first = self.post_verification(2, "vf-partial-1")
        self.assertEqual(status, 201, first)
        first_payload = json.loads(first)
        self.assertEqual(first_payload["anchorSeqBound"], 1)
        self.assertFalse(first_payload["fullyCovered"])
        self.assertEqual(first_payload["uncoveredNodes"], [2])
        # 首份报告冻结锚点上界为 1：补锚后新报告取新上界，旧报告字节不变。
        self.create_anchor(2, "vf-pb-a2")
        status, second = self.post_verification(2, "vf-partial-2")
        self.assertEqual(status, 201, second)
        second_payload = json.loads(second)
        self.assertEqual(second_payload["verificationSeq"], 2)
        self.assertEqual(second_payload["anchorSeqBound"], 2)
        self.assertTrue(second_payload["fullyCovered"])
        self.assertEqual(second_payload["uncoveredNodes"], [])
        status, body = self.get_verifications("/v1/audit-verifications?limit=2")
        self.assertEqual(status, 200, body)
        reports = json.loads(body)["verifications"]
        self.assertEqual(reports[0], first_payload)

    def test_anchor_pointing_outside_bound_is_ignored(self) -> None:
        self.create_comparison(1, "vf-ob-1")
        self.create_comparison(2, "vf-ob-2")
        self.create_comparison(3, "vf-ob-3")
        self.create_anchor(3, "vf-ob-a3")
        status, response = self.post_verification(1, "vf-outside")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        # 锚点序号 1 指向链位置 3：超出比较上界 1，既不算有效覆盖也不记异常。
        self.assertEqual(payload["anchorSeqBound"], 1)
        self.assertEqual(payload["invalidAnchors"], [])
        self.assertEqual(payload["uncoveredNodes"], [1])
        self.assertFalse(payload["fullyCovered"])

    def test_chain_digest_recomputed_over_dense_history(self) -> None:
        for index in range(1, 5):
            self.create_comparison(index, f"vf-dense-{index}")
        for index in range(1, 5):
            self.create_anchor(index, f"vf-dense-a{index}")
        status, response = self.post_verification(4, "vf-dense")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertTrue(payload["chainConsistent"])
        self.assertTrue(payload["fullyCovered"])
        self.assertEqual(payload["anchorSeqBound"], 4)

    # ---- 异常检测：链缺失、替换、重排 ----

    def test_tampered_chain_digest_is_recorded_not_fixed(self) -> None:
        self.create_comparison(1, "vf-tc-1")
        self.create_comparison(2, "vf-tc-2")
        self.create_comparison(3, "vf-tc-3")
        self.create_anchor(1, "vf-tc-a1")
        self.create_anchor(2, "vf-tc-a2")
        self.create_anchor(3, "vf-tc-a3")
        tampered = "ab" * 32
        self.db_execute(
        ...
        status, response = self.post_verification(3, "vf-tampered-chain")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertFalse(payload["chainConsistent"])
        self.assertEqual(payload["invalidChainNodes"], [2])
        # 异常不级联：节点 1、3 仍按比较真相重算通过；历史数据未被修正。
    ...
    def test_deleted_chain_node_is_recorded(self) -> None:
        self.create_comparison(1, "vf-dl-1")
        self.create_comparison(2, "vf-dl-2")
        self.create_anchor(1, "vf-dl-a1")
        self.create_anchor(2, "vf-dl-a2")
        self.db_execute("DELETE FROM audit_comparison_chain WHERE comparison_seq = 1")
        status, response = self.post_verification(2, "vf-deleted-chain")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertEqual(payload["invalidChainNodes"], [1])
        # 指向缺失节点的锚点无效；节点 2 的链记录按真相仍重算通过。
        self.assertEqual(payload["invalidAnchors"], [1])
        self.assertEqual(payload["uncoveredNodes"], [1])
        self.assertFalse(payload["chainConsistent"])
        self.assertFalse(payload["fullyCovered"])

    def test_reordered_chain_rows_are_recorded(self) -> None:
        self.create_comparison(1, "vf-rr-1")
        self.create_comparison(2, "vf-rr-2")
        digest_one = self.chain_digest(1)
        digest_two = self.chain_digest(2)
        # 交换两条链记录的链摘要：连续性、前项或链摘要核对均应失败。
        self.db_execute(
            "UPDATE audit_comparison_chain SET chain_digest = ?"
        ...
        status, response = self.post_verification(2, "vf-reordered")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertEqual(payload["invalidChainNodes"], [1, 2])
        self.assertFalse(payload["chainConsistent"])

    def test_tampered_comparison_response_is_recorded(self) -> None:
        self.create_comparison(1, "vf-tr-1")
        self.create_anchor(1, "vf-tr-a1")
        self.db_execute(
            "UPDATE audit_comparisons SET response_json = ? WHERE comparison_seq = 1",
            ('{"tampered":true}',),
        )
        status, response = self.post_verification(1, "vf-tampered-response")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        # 重算响应摘要与保存链记录不符：节点 1 无效，锚点覆盖的是旧链摘要。
        self.assertEqual(payload["invalidChainNodes"], [1])
        self.assertFalse(payload["chainConsistent"])

    def test_previous_digest_tamper_keeps_anchor_valid_but_chain_invalid(self) -> None:
        self.create_comparison(1, "vf-pt-1")
        self.create_comparison(2, "vf-pt-2")
        self.create_anchor(1, "vf-pt-a1")
        self.create_anchor(2, "vf-pt-a2")
        # 仅篡改节点 2 保存的前项摘要（不动其 chainDigest）：链核对失败，
        # 但锚点只冻结 chainDigest，仍为有效覆盖，两项结论相互独立。
        self.db_execute(
            "UPDATE audit_comparison_chain SET previous_chain_digest = ?"
            " WHERE comparison_seq = 2",
            ("ab" * 32,),
        )
        status, response = self.post_verification(2, "vf-prev-tamper")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertEqual(payload["invalidChainNodes"], [2])
        self.assertFalse(payload["chainConsistent"])
    ...
    # ---- 异常检测：锚点签名、身份、公钥摘要、链位置 ----

    def test_tampered_anchor_signature_is_invalid(self) -> None:
        self.create_comparison(1, "vf-as-1")
        self.create_anchor(1, "vf-as-a1")
        self.db_execute(
            "UPDATE audit_anchors SET auth_signature = ? WHERE anchor_seq = 1",
            ("ab" * 64,),
        )
        status, response = self.post_verification(1, "vf-bad-sig")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertTrue(payload["chainConsistent"])
        self.assertEqual(payload["invalidAnchors"], [1])
    ...
    def test_tampered_anchor_identity_is_invalid(self) -> None:
        self.create_comparison(1, "vf-ai-1")
        self.create_anchor(1, "vf-ai-a1")
        self.db_execute(
            "UPDATE audit_anchors SET auditor_machine_id = ? WHERE anchor_seq = 1",
            ("cd" * 32,),
        )
        status, response = self.post_verification(1, "vf-bad-identity")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertEqual(payload["invalidAnchors"], [1])
        self.assertEqual(payload["uncoveredNodes"], [1])

    def test_tampered_anchor_public_key_is_invalid(self) -> None:
        self.create_comparison(1, "vf-ap-1")
        self.create_anchor(1, "vf-ap-a1")
        self.db_execute(
            "UPDATE audit_anchors SET public_key = ? WHERE anchor_seq = 1",
            ("ee" * 32,),
        )
        status, response = self.post_verification(1, "vf-bad-public")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        # 公钥摘要不再等于审计身份（签名同时无法复核）：锚点无效。
        self.assertEqual(payload["invalidAnchors"], [1])
        self.assertEqual(payload["uncoveredNodes"], [1])

    def test_tampered_anchor_chain_digest_is_invalid(self) -> None:
        self.create_comparison(1, "vf-ad-1")
        self.create_anchor(1, "vf-ad-a1")
        self.db_execute(
            "UPDATE audit_anchors SET chain_digest = ? WHERE anchor_seq = 1",
            ("ab" * 32,),
        )
        status, response = self.post_verification(1, "vf-bad-anchor-digest")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertEqual(payload["invalidAnchors"], [1])
        self.assertEqual(payload["uncoveredNodes"], [1])

    def test_tampered_anchor_auth_fields_fail_rebuilt_signature(self) -> None:
        self.create_comparison(1, "vf-af-1")
        self.create_anchor(1, "vf-af-a1")
        self.db_execute(
            "UPDATE audit_anchors SET key_version = key_version + 1"
            " WHERE anchor_seq = 1"
        )
        status, response = self.post_verification(1, "vf-bad-fields")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertEqual(payload["invalidAnchors"], [1])

    def test_one_valid_anchor_suffices_for_coverage(self) -> None:
        auditor2_seed = b"\x0a" * 32
        auditor2_public = _ed25519_public_key(auditor2_seed).hex()
        auditor2_id = machine_id(auditor2_public)
        status, _ = self.post_json(
            "/v1/machines", {"publicKey": auditor2_public}, "vf-va-register"
        )
        self.assertEqual(status, 201)
        self.server.auditors = frozenset({self.auditor_id, auditor2_id})
        self.create_comparison(1, "vf-va-1")
        first = self.create_anchor(1, "vf-va-a1")
        self.assertEqual(first["anchorSeq"], 1)
        body = json.dumps(
            {"comparisonSeq": 1, "chainDigest": self.chain_digest(1)}
        ).encode()
        status, second = self.request_raw(
            ...
            seed=auditor2_seed,
            actor=auditor2_id,
            nonce=f"nonce-vf-va-{time.time_ns()}",
        )
        self.assertEqual(status, 201, second)
        # 篡改锚点 1 的签名：锚点 1 记为异常，但锚点 2 仍有效覆盖节点 1。
        self.db_execute(
            "UPDATE audit_anchors SET auth_signature = ? WHERE anchor_seq = 1",
            ("ab" * 64,),
        )
        status, response = self.post_verification(1, "vf-valid-anchor")
        self.assertEqual(status, 201, response)
        payload = json.loads(response)
        self.assertEqual(payload["anchorSeqBound"], 2)
        self.assertEqual(payload["invalidAnchors"], [1])
    ...
    def test_replay_returns_first_bytes_without_advancing_seq(self) -> None:
        ...
        status, first = self.post_verification(1, "vf-replay")
        self.assertEqual(status, 201)
        status, second = self.post_verification(1, "vf-replay")
        self.assertEqual(status, 201)
        self.assertEqual(second, first)
        status, third = self.post_verification(1, "vf-replay-other")
        self.assertEqual(status, 201, third)
        self.assertEqual(json.loads(third)["verificationSeq"], 2)

    def test_replay_survives_restart(self) -> None:
        self.create_comparison(1, "vf-rs-1")
        status, first = self.post_verification(1, "vf-restart")
        self.assertEqual(status, 201)
        self.restart()
        status, second = self.post_verification(1, "vf-restart")
        self.assertEqual(status, 201)
        self.assertEqual(second, first)

    def test_same_key_different_body_or_auth_conflicts(self) -> None:
        self.create_comparison(1, "vf-sk-1")
        self.create_comparison(2, "vf-sk-2")
        status, _ = self.post_verification(1, "vf-same-key")
        self.assertEqual(status, 201)
        status, body = self.post_verification(2, "vf-same-key")
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "conflict"})
        status, body = self.post_verification(
            1, "vf-same-key", nonce=f"nonce-vf-sk-{time.time_ns()}"
        )
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "conflict"})
        # 同键更换正文即使目标比较不存在也先判冲突。
        status, body = self.post_verification(999, "vf-same-key")
        self.assertEqual(status, 409)

    def test_missing_comparison_is_not_found(self) -> None:
        self.create_comparison(1, "vf-nf-1")
        for seq in (2, 999, 10**30):
            status, body = self.post_verification(seq, f"vf-missing-{seq}")
    ...
    def test_replayed_nonce_with_different_key_conflicts(self) -> None:
        self.create_comparison(1, "vf-nr-1")
        nonce = f"nonce-vf-nr-{time.time_ns()}"
        status, _ = self.post_verification(1, "vf-nr-1", nonce=nonce)
        self.assertEqual(status, 201)
        status, body = self.post_verification(1, "vf-nr-2", nonce=nonce)
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "replay_detected"})
    ...
    def test_concurrent_requests_get_dense_unique_seqs(self) -> None:
        ...
        def verify(index: int) -> tuple[int, bytes]:
            return self.post_verification(
                1,
                f"vf-cc-{index}",
                nonce=f"nonce-vf-conc-{index:020d}",
            )

        with concurrent.futures.ThreadPoolExecutor(count) as executor:
            results = list(executor.map(verify, range(1, count + 1)))
        self.assertTrue(all(status == 201 for status, _ in results), results)
        seqs = sorted(
            json.loads(response)["verificationSeq"] for _, response in results
        )
        self.assertEqual(seqs, list(range(1, count + 1)))

    def test_concurrent_same_key_single_success(self) -> None:
        self.create_comparison(1, "vf-cs-1")

        def verify(_index: int) -> tuple[int, bytes]:
            return self.post_verification(
                1,
                "vf-cs-same",
                nonce=f"nonce-vf-csame-{_index:020d}",
            )
    ...
    def test_invalid_body_returns_400(self) -> None:
        ...
        for index, body in enumerate(bodies):
            status, response = self.request_raw(
                "/v1/audit-verifications", body, f"vf-invalid-{index}", "POST"
            )
            self.assertEqual(status, 400, body)
            self.assertEqual(json.loads(response), {"error": "invalid_request"})
    ...
    def test_query_params_delegation_and_missing_auth_rejected(self) -> None:
        ...
        status, response = self.request_raw(
            "/v1/audit-verifications?x=1", body, "vf-query", "POST"
        )
        self.assertEqual(status, 400)
        self.assertEqual(json.loads(response), {"error": "invalid_request"})
        ...
        self.assertEqual(status, 400)
        status, _ = self.request_raw(
            "/v1/audit-verifications", body, "vf-no-auth", "POST", omit_auth=True
        )
        self.assertEqual(status, 400)
        request = Request(
    ...
    def test_stale_request_is_401(self) -> None:
        self.create_comparison(1, "vf-st-1")
        stale = int(time.time() * 1000) - 400_000
        status, body = self.post_verification(
            1, "vf-stale", request_time_ms=stale
        )
        self.assertEqual(status, 401)
        self.assertEqual(json.loads(body), {"error": "stale_request"})

    def test_invalid_signature_is_401(self) -> None:
        self.create_comparison(1, "vf-is-1")
        body = json.dumps({"comparisonSeq": 1}).encode()
        good = make_sla_auth(
            self.server,
            "vf-bad-signature",
    ...
    def test_get_collection_returns_full_reports(self) -> None:
        ...
        self.assertEqual(status, 201)
        self.create_anchor(1, "vf-gc-a1")
        status, second = self.post_verification(1, "vf-gc-2")
        self.assertEqual(status, 201)
        status, body = self.get_verifications()
        self.assertEqual(status, 200, body)
        payload = json.loads(body)
        self.assertEqual([r["verificationSeq"] for r in payload["verifications"]], [1, 2])
        self.assertEqual(payload["verifications"][0], json.loads(first))
        self.assertEqual(payload["verifications"][1], json.loads(second))
        self.assertIsNone(payload["nextCursor"])

    def test_collection_paging_and_stable_cut(self) -> None:
        self.create_comparison(1, "vf-pg-1")
        for index in range(1, 4):
            status, _ = self.post_verification(1, f"vf-pg-{index}")
            self.assertEqual(status, 201)
        status, body = self.get_verifications("/v1/audit-verifications?limit=1")
        self.assertEqual(status, 200, body)
        first_page = json.loads(body)
        self.assertEqual([r["verificationSeq"] for r in first_page["verifications"]], [1])
        self.assertEqual(first_page["nextCursor"], "3:1")
        status, _ = self.post_verification(1, "vf-pg-4")
        self.assertEqual(status, 201)
        status, body = self.get_verifications(
        ...
        self.assertEqual(status, 200, body)
        second_page = json.loads(body)
        self.assertEqual([r["verificationSeq"] for r in second_page["verifications"]], [2])
        self.assertEqual(second_page["nextCursor"], "3:2")
    ...
    def test_collection_invalid_params_and_cursors(self) -> None:
        ...
        for query in (
            ...
            status, _ = self.get_verifications(f"/v1/audit-verifications?{query}")
            self.assertEqual(status, 400, query)
    ...
    def test_collection_requires_auditor_and_auth(self) -> None:
        ...
        self.assertEqual(status, 400)
        request = Request(self.url("/v1/audit-verifications"), data=b"", method="GET")
        request.add_header("SLA-Delegation", "d;0;1;n;s")
        try:
            with urlopen(request, timeout=5) as response:
                status = response.status
    ...
    def test_collection_nonce_consumed_only_on_success(self) -> None:
        nonce = f"nonce-vf-gn-{time.time_ns()}"
        self.create_comparison(1, "vf-gn-1")
        status, _ = self.get_verifications(nonce=nonce)
        self.assertEqual(status, 200)
        status, body = self.get_verifications(nonce=nonce)
        self.assertEqual(status, 409)
        self.assertEqual(json.loads(body), {"error": "replay_detected"})
        failed_nonce = f"nonce-vf-gnf-{time.time_ns()}"
        for _ in range(2):
            status, _ = self.get_verifications(
                "/v1/audit-verifications?cursor=99:1", nonce=failed_nonce
            )
            self.assertEqual(status, 400)
...
class EvidenceProofTests(unittest.TestCase):
    ...
    def issue_proof_delegation(
        ...
        *,
        ...
        if operation == "evidence.proof.write":
            fields["evidenceSeq"] = (
                self.evidence_seq if evidence_seq is None else evidence_seq
            )
        elif operation == "evidence.write":
            fields["disputeId"] = dispute_id
        else:
            fields["capabilityVersion"] = capability_version
        return self.post_with_auth(
            "/v1/delegations", json.dumps(fields).encode(), key,
            seed=seed or self.CONSUMER_SEED,
            actor=actor or self.consumer_id,
        )

    def submit_delegated_proof(
        self,
        body: dict[str, object] | None = None,
        key: str = "dpf-1",
        *,
        ...
        if body is None:
            body = self.proof_body(self.CONSUMER_SEED)
        return self.post_with_auth(
            "/v1/evidence-proofs", json.dumps(body).encode(), key,
            seed=self.CONSUMER_SEED, actor=self.consumer_id,
            delegation=(seed, delegation_id),
            request_time_ms=request_time_ms, nonce=nonce,
        )
    ...
    def get_audit(self, path: str) -> tuple[int, object]:
        ...
        request.add_header(
            "SLA-Auth",
            make_sla_auth(
                self.server, None, self.CONSUMER_SEED, self.consumer_id,
                "GET", urlsplit(path).path, b"", 1,
            ),
        )
        try:
    ...
    def test_issue_proof_scope_persisted_and_listed(self) -> None:
        ...
        self.assertEqual(list(body), ["id", "expiresAt"])
        self.assertEqual(body["id"], "del-pf-1")
        connection = sqlite3.connect(self.server.database_path)
        try:
            row = connection.execute(
                "SELECT operation, capability_version, dispute_id, evidence_seq"
                " FROM machine_delegations WHERE id = 'del-pf-1'"
            ).fetchone()
            self.assertEqual(
                row, ("evidence.proof.write", None, None, self.evidence_seq)
            )
        finally:
            connection.close()
        status, listed = self.get_audit(
            f"/v1/machines/{self.consumer_id}/delegations"
        )
        self.assertEqual(status, 200)
        item = listed["delegations"][0]
        self.assertEqual(
        ...
        self.assertEqual(
            (
                ...
                item["disputeId"], item["evidenceSeq"],
            ),
    ...
    def test_issue_proof_scope_invalid_bodies(self) -> None:
        ...
        for index, fields in enumerate(cases):
            status, payload = self.post_with_auth(
    ...
    def test_delegate_submits_proof_created(self) -> None:
        ...
        status, payload = self.submit_delegated_proof()
        self.assertEqual(status, 201)
        body = json.loads(payload)
        self.assertEqual(list(body), ["proofSeq", "verified", "createdAt"])
        self.assertEqual((body["proofSeq"], body["verified"]), (1, True))
        self.assertEqual(self.consumed_flag("del-pf-1"), 1)
        # consumed 事件同事务记录标准路径与原始正文摘要。
        connection = sqlite3.connect(self.server.database_path)
        try:
            event = connection.execute(
                "SELECT type, resource, request_digest FROM delegation_events"
                " WHERE delegation_id = 'del-pf-1' AND type = 'consumed'"
            ).fetchone()
        finally:
            connection.close()
        expected_body = json.dumps(self.proof_body(self.CONSUMER_SEED)).encode()
        self.assertEqual(event[0], "consumed")
        self.assertEqual(event[1], "/v1/evidence-proofs")
        self.assertEqual(event[2], hashlib.sha256(expected_body).hexdigest())

    def test_delegate_proof_replay_same_key_and_restart(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        status, first = self.submit_delegated_proof()
        self.assertEqual(status, 201)
        self.assertEqual(self.submit_delegated_proof(), (201, first))
        self.restart()
        self.assertEqual(self.submit_delegated_proof(), (201, first))
        status, payload = self.get_json(f"/v1/evidence/{self.evidence_seq}/proofs")
        self.assertEqual(len(payload["proofs"]), 1)

    def test_delegate_proof_same_key_different_body_or_auth_conflicts(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        body = self.proof_body(self.CONSUMER_SEED)
        self.assertEqual(self.submit_delegated_proof(body, "dpf-1")[0], 201)
        # 同键异正文。
        other = self.proof_body(self.CONSUMER_SEED, signature="ab" * 64)
        status, payload = self.submit_delegated_proof(other, "dpf-1")
        self.assertEqual((status, json.loads(payload)), (409, {"error": "conflict"}))
        # 同键异认证五段。
        status, payload = self.submit_delegated_proof(
            body, "dpf-1", nonce="nonce-proof-other-1"
        )
        self.assertEqual((status, json.loads(payload)), (409, {"error": "conflict"}))

    def test_delegate_proof_both_auth_headers_400_before_evidence_lookup(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        # 证据不存在，但两种认证头并存须先返回 400。
        body = json.dumps(self.proof_body(self.CONSUMER_SEED, evidence_seq=999999)).encode()
        status, payload = self.post_with_auth(
            "/v1/evidence-proofs", body, "dpf-both",
            seed=self.CONSUMER_SEED, actor=self.consumer_id,
            delegation=(DELEGATE_SEED, "del-pf-1"), both=True,
        )
        self.assertEqual(
            (status, json.loads(payload)), (400, {"error": "invalid_request"})
        )

    def test_delegate_proof_missing_auth_400(self) -> None:
        body = json.dumps(self.proof_body(self.CONSUMER_SEED)).encode()
        status, payload = self.post_with_auth(
            "/v1/evidence-proofs", body, "dpf-noauth",
            seed=self.CONSUMER_SEED, actor=self.consumer_id, no_auth=True,
        )
        self.assertEqual(
            (status, json.loads(payload)), (400, {"error": "invalid_request"})
        )

    def test_delegate_proof_unknown_delegation_401(self) -> None:
        status, payload = self.submit_delegated_proof(delegation_id="del-ghost")
        self.assertEqual(
            (status, json.loads(payload)), (401, {"error": "invalid_authentication"})
        )

    def test_delegate_proof_wrong_operation_or_scope_403(self) -> None:
        # evidence.write 凭证不得用于证明入口。
        self.assertEqual(
            self.issue_proof_delegation(
                "del-ev", key="issue-ev", operation="evidence.write"
            )[0],
            201,
        )
        status, payload = self.submit_delegated_proof(
            key="dpf-ev", delegation_id="del-ev"
        )
        self.assertEqual((status, json.loads(payload)), (403, {"error": "forbidden"}))
        # capability.write 凭证不得用于证明入口。
        self.assertEqual(
            self.issue_proof_delegation(
                "del-cap", key="issue-cap", operation="capability.write"
            )[0],
            201,
        )
        status, payload = self.submit_delegated_proof(
            key="dpf-cap", delegation_id="del-cap"
        )
        self.assertEqual((status, json.loads(payload)), (403, {"error": "forbidden"}))
        # 证明凭证的证据序号与正文不符。
        other_seq = self.add_evidence("ev-2", "cd" * 32, "evidence-2")
        self.assertEqual(
            self.issue_proof_delegation(
                "del-pf-2", key="issue-pf-2", evidence_seq=other_seq
            )[0],
            201,
        )
        status, payload = self.submit_delegated_proof(
            key="dpf-2", delegation_id="del-pf-2"
        )
        self.assertEqual((status, json.loads(payload)), (403, {"error": "forbidden"}))
        # 范围不符不消费任何凭证。
        for delegation_id in ("del-ev", "del-cap", "del-pf-2"):
            self.assertEqual(self.consumed_flag(delegation_id), 0)

    def test_delegate_proof_issuer_mismatch_403(self) -> None:
        # 凭证由消费者签发，正文 actorId 为生产者：签发身份越界。
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        body = self.proof_body(self.PRODUCER_SEED)
        status, payload = self.submit_delegated_proof(body, "dpf-mis")
        self.assertEqual((status, json.loads(payload)), (403, {"error": "forbidden"}))
        self.assertEqual(self.consumed_flag("del-pf-1"), 0)

    def test_delegate_proof_stranger_actor_403(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        body = self.proof_body(self.STRANGER_SEED)
        status, payload = self.submit_delegated_proof(body, "dpf-str")
        self.assertEqual((status, json.loads(payload)), (403, {"error": "forbidden"}))

    def test_delegate_proof_stale_401_then_retry(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        status, payload = self.submit_delegated_proof(
            request_time_ms=int(time.time() * 1000) - 400_000
        )
        self.assertEqual((status, json.loads(payload)), (401, {"error": "stale_request"}))
        # 过时不消费凭证、随机数或序号，随后可正常成功。
        self.assertEqual(self.consumed_flag("del-pf-1"), 0)
        self.assertEqual(self.submit_delegated_proof(key="dpf-2")[0], 201)

    def test_delegate_proof_bad_delegate_signature_401_then_retry(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        status, payload = self.submit_delegated_proof(seed=PUBLIC_KEY_SEED_C)
        self.assertEqual(
            (status, json.loads(payload)), (401, {"error": "invalid_authentication"})
        )
        # 凭证失败后仍可重试。
    ...
    def test_delegate_proof_expired_401(self) -> None:
        now = int(time.time() * 1000)
        connection = sqlite3.connect(self.server.database_path)
        try:
            connection.execute(
                ...
                "(id, issuer_machine_id, delegate_public_key, expires_at_ms,"
                " issued_key_version, revoked, consumed, created_at_ms,"
                " operation, evidence_seq)"
                " VALUES (?, ?, ?, ?, 1, 0, 0, ?, 'evidence.proof.write', ?)",
                ("del-exp", self.consumer_id, DELEGATE_PUBLIC, now - 1, now - 1000,
                 self.evidence_seq),
            )
            connection.commit()
        finally:
            connection.close()
        status, payload = self.submit_delegated_proof(delegation_id="del-exp")
        self.assertEqual(
            (status, json.loads(payload)), (401, {"error": "invalid_authentication"})
        )

    def test_delegate_proof_consumed_credential_401(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        self.assertEqual(self.submit_delegated_proof()[0], 201)
        status, payload = self.submit_delegated_proof(key="dpf-2")
        self.assertEqual(
            (status, json.loads(payload)), (401, {"error": "invalid_authentication"})
        )

    def test_delegate_proof_revoked_401(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        status, _ = self.post_with_auth(
            "/v1/delegations/del-pf-1/revocation", b"{}", "revoke-pf-1",
            seed=self.CONSUMER_SEED, actor=self.consumer_id,
        )
        self.assertEqual(status, 200)
        status, payload = self.submit_delegated_proof()
    ...
    def test_delegate_proof_resolved_dispute_409_not_consumed(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        status, _ = self.post_json(
    ...
    def test_delegate_proof_bad_business_signature_409_not_consumed(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        body = self.proof_body(self.CONSUMER_SEED, signature="ab" * 64)
        status, payload = self.submit_delegated_proof(body)
        self.assertEqual(
            (status, json.loads(payload)), (409, {"error": "invalid_signature"})
        )
        self.assertEqual(self.consumed_flag("del-pf-1"), 0)

    def test_delegate_proof_duplicate_signer_409_after_business_verify(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        # 直接认证先行证明；代理重复证明在业务验签后判定。
        self.assertEqual(self.post_proof()[0], 201)
        status, payload = self.submit_delegated_proof()
        self.assertEqual((status, json.loads(payload)), (409, {"error": "proof_exists"}))
        self.assertEqual(self.consumed_flag("del-pf-1"), 0)

    def test_proof_nonce_reuse_different_key_409(self) -> None:
        nonce = "nonce-proof-reuse-direct"
        body = json.dumps(self.proof_body(self.CONSUMER_SEED)).encode()
        status, _ = self.post_with_auth(
            "/v1/evidence-proofs", body, "pf-n1",
            seed=self.CONSUMER_SEED, actor=self.consumer_id, nonce=nonce,
        )
        self.assertEqual(status, 201)
        # 有效随机数异键复用：不留幂等记录。
        status, payload = self.post_with_auth(
            "/v1/evidence-proofs", body, "pf-n2",
            seed=self.CONSUMER_SEED, actor=self.consumer_id, nonce=nonce,
        )
        self.assertEqual(
            (status, json.loads(payload)), (409, {"error": "replay_detected"})
        )
        # pf-n2 未留幂等记录：换新随机数后进入业务判定（同签名者重复证明）。
        status, payload = self.post_with_auth(
            "/v1/evidence-proofs", body, "pf-n2",
            seed=self.CONSUMER_SEED, actor=self.consumer_id,
        )
    ...
    def test_delegate_proof_concurrent_single_winner(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        results: list[int] = []
        lock = threading.Lock()

        def submit(index: int) -> None:
            status, _ = self.submit_delegated_proof(key=f"dpf-race-{index}")
            with lock:
                results.append(status)
    ...
    def test_delegate_proof_consumption_audit(self) -> None:
        self.assertEqual(self.issue_proof_delegation()[0], 201)
        self.assertEqual(self.submit_delegated_proof()[0], 201)
        status, audit = self.get_audit(
            f"/v1/machines/{self.consumer_id}/delegation-consumptions"
        )
        self.assertEqual(status, 200)
        record = audit["consumptions"][0]
        ...
        self.assertIsNone(record["capabilityVersion"])
        self.assertEqual(record["evidenceSeq"], self.evidence_seq)
        self.assertEqual(record["resource"], "/v1/evidence-proofs")
        self.assertEqual(
            record["requestDigest"],
            hashlib.sha256(
                json.dumps(self.proof_body(self.CONSUMER_SEED)).encode()
            ).hexdigest(),
        )
        self.assertGreater(record["createdAt"], 0)
...
class DelegationProofMigrationTests(unittest.TestCase):
    ...
    def test_evidence_seq_column_added_once_and_legacy_rows_null(self) -> None:
        ...
        connection = database_connect(self.database_path)
        try:
            # 能力授权、单笔争议授权与无范围凭证各一条，含 evidence_seq 新列写入。
            connection.execute(
                "INSERT INTO machine_delegations"
                "(id, issuer_machine_id, delegate_public_key, expires_at_ms,"
                " issued_key_version, revoked, consumed, created_at_ms,"
            ...
            connection.execute(
                ...
                " issued_key_version, revoked, consumed, created_at_ms,"
                " operation, capability_version, dispute_id, evidence_seq)"
                " VALUES (?, ?, ?, ?, 1, 0, 0, ?, 'evidence.write', NULL, 'dispute-1', NULL)",
        ...
        try:
            ...
            rows = connection.execute(
                "SELECT id, operation, dispute_id, evidence_seq"
                " FROM machine_delegations ORDER BY id"
            ).fetchall()
            self.assertEqual(
                [tuple(row) for row in rows],
                [
                    ("d-cap", "capability.write", None, None),
                    ("d-ev", "evidence.write", "dispute-1", None),
                    ("d-legacy", None, None, None),
                ],
            )
            marker = connection.execute(
