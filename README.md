...
- `POST /v1/slas/{id}/evaluations`：评估 active 状态 SLA 在指定区间的履约结果。请求须携带 `Idempotency-Key` 头（格式同上），JSON 请求体仅含且不得重复 `from`、`to`，均为非布尔整数且 `from<to`，并须满足 `start*1000<=from<to<=end*1000`。事务起点取全库最大 `commit_seq` 为 `cut`（空库为 `0`），仅聚合该 SLA 内 `commit_seq<=cut` 且 `timestamp∈[from,to)` 的事件，`count`、`latencySum`、`maxLatency`、`violations` 定义沿用遥测查询。`outcome` 在 `count=0` 时为 `insufficient`，否则 `violations=0` 时为 `fulfilled`，其余为 `breached`。评估快照与幂等结果原子持久化，后续遥测不得改变结果。首次返回 `201`，正文键序 `from,to,cut,count,latencySum,maxLatency,violations,outcome`；`outcome` 为字符串，`maxLatency` 空时为 `null`，其余字段为非负整数。判定顺序为头、体、幂等记录、SLA、状态/区间：头或体非法返回 `400`/`invalid_request`；同键异 id 或异体返回 `409`/`conflict`；SLA 不存在返回 `404`/`not_found`；非 active 或区间越界返回 `409`/`conflict`。同键同请求并发或重启均重放首次状态码与正文字节；失败不留记录。
