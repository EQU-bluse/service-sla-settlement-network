...
CREATE TABLE IF NOT EXISTS dispute_evidence_snapshots (
    ...
    dispute_id TEXT NOT NULL,
    evidence_seq_bound INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS dispute_adjudication_proposal_idempotency_records (
    key TEXT PRIMARY KEY,
    dispute_id TEXT NOT NULL,
    request_json TEXT NOT NULL,
    status INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS dispute_escalations (
    escalation_seq INTEGER PRIMARY KEY,
    dispute_id TEXT NOT NULL UNIQUE,
    first_proposal_seq INTEGER NOT NULL,
    second_proposal_seq INTEGER NOT NULL,
    first_snapshot_digest TEXT NOT NULL,
    second_snapshot_digest TEXT NOT NULL,
    deadline_ms INTEGER NOT NULL,
    amount_micros INTEGER NOT NULL,
    created_at_ms INTEGER NOT NULL
);
CREATE TABLE IF NOT EXISTS dispute_escalation_idempotency_records (
    key TEXT PRIMARY KEY,
    dispute_id TEXT NOT NULL,
    request_json TEXT NOT NULL,
    status INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS dispute_arbitrations (
    arbitration_seq INTEGER PRIMARY KEY,
    dispute_id TEXT NOT NULL UNIQUE,
    escalation_seq INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS dispute_arbitration_idempotency_records (
    key TEXT PRIMARY KEY,
    dispute_id TEXT NOT NULL,
    request_json TEXT NOT NULL,
    status INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS audit_checkpoints (
    ...
    entry_seq_bound INTEGER NOT NULL,
    digest TEXT NOT NULL,
    created_by TEXT NOT NULL,
    created_at_ms INTEGER NOT NULL,
    document_json TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS audit_checkpoint_idempotency_records (
    key TEXT PRIMARY KEY,
    request_json TEXT NOT NULL,
    status INTEGER NOT NULL,
    response_json TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS audit_anchors (
    ...
    created_at_ms INTEGER NOT NULL,
    response_json TEXT NOT NULL,
    UNIQUE (comparison_seq, auditor_machine_id)
);
...
CREATE TABLE IF NOT EXISTS audit_verifications (
    ...
    comparison_seq_bound INTEGER NOT NULL,
    anchor_seq_bound INTEGER NOT NULL,
    digest TEXT NOT NULL,
    created_by TEXT NOT NULL,
    created_at_ms INTEGER NOT NULL,
    response_json TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS audit_verification_idempotency_records (
    key TEXT PRIMARY KEY,
    request_json TEXT NOT NULL,
    status INTEGER NOT NULL,
    response_json TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_dispute_escalations_dispute_seq
ON dispute_escalations(dispute_id, escalation_seq);
CREATE INDEX IF NOT EXISTS idx_dispute_arbitrations_dispute_seq
ON dispute_arbitrations(dispute_id, arbitration_seq);
...
def _add_delegation_proof(connection: sqlite3.Connection) -> None:
    # 证明授权：旧委托只补可空 evidence_seq 列并保持 NULL。
    # 能力授权、单笔争议授权与升级前凭证对应 null；不改动旧委托、事件序号、
    # 幂等数据或历史响应字节。
    connection.execute("BEGIN IMMEDIATE")
    try:
        if (
...
def _backfill_audit_comparison_chain(connection: sqlite3.Connection) -> None:
    # 比较链一次性补链：旧比较按比较序号升序在单事务内逐行计算链摘要；
    ...
    try:
        if (
            ...
            connection.execute("COMMIT")
            return
        previous_chain_digest = AUDIT_CHAIN_GENESIS_DIGEST
        rows = connection.execute(
